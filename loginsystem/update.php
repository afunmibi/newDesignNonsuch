...
if (isset($_POST['update_enrollee_details'])) {
  ...
  if (!move_uploaded_file($file_tmp, $file_store)) {
    ...
    $stmt = $con->prepare("UPDATE enrolment SET organization=?, sname=?, oname=?, policy_no=?, phone_no=?, email=?, e_provider=?, gender=?, plan_type=?, e_location=?, dob=?, no_of_dependant=?, reg_status=?, alternate_no=?, date_captured=?, photo=? WHERE id=?");
...
<body>
  ...
  <div class="row">
    ...
    <div class="col-md-4 col-sm-6 mb-2">
       ...
       <input type="text" name="location" class="form-control"  id="location" placeholder="Location" value="<?php echo $row['e_location'] ?>">
    </div>
      </div>
       <div class="row">
    <div class="col-md-4 col-sm-6 mb-2">
    ...
    <div class="col-md-4 col-sm-6 mb-2">
      ...
      <input type="text" name="provider" class="form-control"  id="provider" placeholder="Provider" value="<?php echo $row['e_provider'] ?>">
    </div>
